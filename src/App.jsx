...
const ChecklistModule = lazy(() => import("./modules/ChecklistModule.jsx"));
const BudgetModule = lazy(() => import("./modules/BudgetModule.jsx"));
const VendorsModule = lazy(() => import("./modules/VendorsModule.jsx"));
const GuiaModule = lazy(() => import("./modules/GuiaModule.jsx"));
...
// ─── GUÍA NUPCIAL Y GUÍA PARA NOVIOS: carga diferida ───────────────────────
...
function GuestsModule({user, onBack, onGoDesigner, isDemo=false}){
  ...
  return <div style={{minHeight:"100dvh",background:"rgba(245,239,224,.88)",paddingBottom:"calc(88px + env(safe-area-inset-bottom))"}}>
    ...
    {guia&&<Suspense fallback={null}><GuiaNupcial abierta={true} seccionInicial={guia} onClose={()=>setGuia(null)} theme={THEME}/></Suspense>}
...
export default function App(){
  ...
  if(view==="guia-novios") return decorate(<><Suspense fallback={<div style={{minHeight:"55vh",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"\'Lora\',serif",color:"rgba(74,94,58,.7)"}}>Cargando guía…</div>}><GuiaModule onBack={()=>setView("home")} isDemo={demo} onRequestPurchase={()=>setPurchaseOpen(true)} theme={THEME} openFullGuideDownload={openFullGuideDownload}/></Suspense>{showNav&&<GlobalNav view={view} setView={setView} hasResults={!!results} isDemo={demo}/>}</>);
  if(view==="budget") return decorate(<><Suspense fallback={<div style={{minHeight:"55vh",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"\'Lora\',serif",color:"rgba(74,94,58,.7)"}}>Cargando presupuesto…</div>}><BudgetModule user={user} onBack={()=>setView("home")} dataClient={dataClient} CATEGORIAS_DEFAULT={CATEGORIAS_DEFAULT} calcBudgetFromVendors={calcBudgetFromVendors} normalizeBudgetCategoryName={normalizeBudgetCategoryName} showToast={showToast} THEME={THEME} CURRENCIES={CURRENCIES} getCurrencySymbol={getCurrencySymbol} fmt={fmt} num={num} BackToHome={BackToHome}/></Suspense>{showNav&&<GlobalNav view={view} setView={setView} hasResults={!!results} isDemo={demo}/>}</>);
  if(view==="vendors") return decorate(<><Suspense fallback={<div style={{minHeight:"55vh",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"\'Lora\',serif",color:"rgba(74,94,58,.7)"}}>Cargando proveedores…</div>}><VendorsModule user={user} onBack={()=>setView("home")} dataClient={dataClient} CATEGORIAS_DEFAULT={CATEGORIAS_DEFAULT} calcBudgetFromVendors={calcBudgetFromVendors} budgetCategoriesToVendorOptions={budgetCategoriesToVendorOptions} vendorAmountInBudgetCurrency={vendorAmountInBudgetCurrency} getCurrencySymbol={getCurrencySymbol} getVendorExchangeRateForBudget={getVendorExchangeRateForBudget} getSuggestedExchangeRate={getSuggestedExchangeRate} formatExchangeRate={formatExchangeRate} CURRENCIES={CURRENCIES} num={num} fmt={fmt} showToast={showToast} loadPrivateExcelTemplateWorkbook={loadPrivateExcelTemplateWorkbook} replaceTemplateTableRows={replaceTemplateTableRows} ensureExcelHowToSheet={ensureExcelHowToSheet} openExcelTemplateDownload={openExcelTemplateDownload} useIsMobile={useIsMobile} isDemoUser={isDemoUser} BackToHome={BackToHome} THEME={THEME} VENDOR_CATS={VENDOR_CATS} VENDOR_ESTADOS={VENDOR_ESTADOS} VENDOR_CATEGORY_EXPORT_LABELS={VENDOR_CATEGORY_EXPORT_LABELS} normalizeSpreadsheetKey={normalizeSpreadsheetKey} VENDOR_CATEGORY_IMPORT_IDS={VENDOR_CATEGORY_IMPORT_IDS} VENDOR_STATE_IMPORT_IDS={VENDOR_STATE_IMPORT_IDS} parseSpreadsheetNumber={parseSpreadsheetNumber} loadSheetJS={loadSheetJS} spreadsheetFileToRows={spreadsheetFileToRows}/></Suspense>{showNav&&<GlobalNav view={view} setView={setView} hasResults={!!results} isDemo={demo}/>}</>);
  ...
  if(view==="checklist-boda") return decorate(<><Suspense fallback={<div style={{minHeight:"55vh",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"\'Lora\',serif",color:"rgba(74,94,58,.7)"}}>Cargando checklist…</div>}><ChecklistModule getDataClient={dataClient} theme={THEME} WeatherWidgetComponent={WeatherWidget} BackToHomeComponent={BackToHome} user={user} form={form} results={results} onGoMusic={()=>setView(results?"results":"guia")} onBack={()=>setView("home")}/></Suspense>{showNav&&<GlobalNav view={view} setView={setView} hasResults={!!results} isDemo={demo}/>}</>);
  ...
  if(view==="results") return decorate(<><Suspense fallback={<div style={{minHeight:"55vh",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"\'Lora\',serif",color:"rgba(74,94,58,.7)"}}>Cargando resultado…</div>}><ResultsModule results={results} form={form} checked={checked} setChecked={(fn)=>{ const next=typeof fn==='function'?fn(checked):fn; setChecked(next); syncChecked(next); }} arquetipo={arquetipo} resultToken={resultToken} onGoHome={()=>setView("home")} onLogout={logout} onRestart={()=>{
    try{localStorage.removeItem("bsb_session");}catch(e){}
    window.history.replaceState({}, "", window.location.pathname);
  ...
  }} theme={THEME} archetypes={ARQUETIPOS} AudioButtonComponent={AudioButton} BackToHomeComponent={BackToHome} showToastFn={showToast}/></Suspense><GlobalNav view={view} setView={setView} hasResults={!!results} isDemo={demo}/><InstallPrompt/></>);
