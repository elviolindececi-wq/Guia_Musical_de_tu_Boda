        ...
        {(()=>{
          ...
          return <div style={{marginTop:16,paddingTop:14,borderTop:"0.5px solid rgba(201,169,110,.15)"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:10}}>
              {catsCotizadas.length>0&&(()=>{
                const positivo = ahorroCot>=0;
                const pct = baseCot>0?Math.round(Math.abs(ahorroCot)/baseCot*100):0;
                return <div style={{background:positivo?"rgba(74,94,58,.07)":"rgba(200,80,60,.06)",border:`0.5px solid ${positivo?"rgba(74,94,58,.25)":"rgba(200,80,60,.25)"}`,borderRadius:10,padding:"12px 14px"}}>
                  ...
                  <div style={{fontFamily:"'Playfair Display',serif",fontSize:"1.15rem",fontWeight:700,color:positivo?"#4A5E3A":"rgba(200,60,60,.8)"}}>
                    {positivo?"▼ ":"▲ "}{SYM}{fmt(Math.abs(ahorroCot))}
                  </div>
                  <div style={{fontFamily:"'Lora',serif",fontSize:".78rem",color:positivo?"rgba(74,94,58,.6)":"rgba(200,60,60,.6)",marginTop:3,lineHeight:1.4}}>
                    {positivo?`${pct}% menos en ${catsCotizadas.length} categoría${catsCotizadas.length>1?"s":""}`:
                    `${pct}% más en ${catsCotizadas.length} categoría${catsCotizadas.length>1?"s":""}`}
                  </div>
                </div>;
              })()}
              {catsPagadas.length>0&&(()=>{
                const positivo = ahorroPag>=0;
                const pct = basePag>0?Math.round(Math.abs(ahorroPag)/basePag*100):0;
                return <div style={{background:positivo?"rgba(74,94,58,.07)":"rgba(200,80,60,.06)",border:`0.5px solid ${positivo?"rgba(74,94,58,.25)":"rgba(200,80,60,.25)"}`,borderRadius:10,padding:"12px 14px"}}>
                  ...
                  <div style={{fontFamily:"'Playfair Display',serif",fontSize:"1.15rem",fontWeight:700,color:positivo?"#4A5E3A":"rgba(200,60,60,.8)"}}>
                    {positivo?"▼ ":"▲ "}{SYM}{fmt(Math.abs(ahorroPag))}
                  </div>
                  <div style={{fontFamily:"'Lora',serif",fontSize:".78rem",color:positivo?"rgba(74,94,58,.6)":"rgba(200,60,60,.6)",marginTop:3,lineHeight:1.4}}>
                    {positivo?`${pct}% menos en ${catsPagadas.length} categoría${catsPagadas.length>1?"s":""}`:
                    `${pct}% más en ${catsPagadas.length} categoría${catsPagadas.length>1?"s":""}`}
                  </div>
                </div>;
              })()}
            </div>
