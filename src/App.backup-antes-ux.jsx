...
.PhoneInput{display:flex;align-items:center;gap:10px;border-bottom:1.5px solid rgba(74,94,58,.3);padding:7px 2px;margin-bottom:12px}
.PhoneInput:focus-within{border-bottom-color:#4A5E3A}
.PhoneInputInput{border:0!important;padding:7px 0!important;margin:0!important;min-height:42px;background:transparent!important}
.PhoneInputCountry{margin-right:4px}
.demo-banner-spacer{height:76px}
@media(max-width:600px){.demo-banner-spacer{height:94px}}
  ...
  return <div style={{background:"rgba(245,239,224,.88)",minHeight:"100dvh",overflow:"hidden"}}>
    <section className="responsive-shell hero-grid" style={{position:"relative"}}>
      ...
      <div style={{position:"relative",zIndex:1}}>
        ...
        <div className="fu4" style={{marginTop:"clamp(30px,5vw,48px)",display:"flex",gap:12,justifyContent:"center",flexWrap:"wrap"}}>
          <button className="pbtn" onClick={onTry}>Probar gratis →</button>
          <button className="gbtn" onClick={onLogin}>Ya compré · Entrar</button>
        </div>
      </div>
    </section>
    ...
    <section className="responsive-shell" style={{paddingBottom:"clamp(42px,8vw,88px)"}}>
      ...
      <div style={{marginTop:"clamp(36px,6vw,64px)",background:"linear-gradient(135deg,#EAE4D2,#F5EFE0)",border:"1px solid rgba(201,169,110,.24)",borderRadius:24,padding:"clamp(28px,5vw,46px)",textAlign:"center"}}>
        ...
        <p style={{fontFamily:"'Lora',serif",fontSize:"clamp(1rem,2vw,1.15rem)",color:DIM,margin:"0 0 24px"}}>Probá la experiencia sin tarjeta. Para guardar y acceder desde cualquier dispositivo, desbloqueá el producto.</p>
        <div style={{display:"flex",gap:12,justifyContent:"center",flexWrap:"wrap"}}>
          <button className="pbtn" onClick={onTry}>Probar gratis →</button>
          <button className="gbtn" onClick={onBuy}>Comprar y guardar</button>
        </div>
      </div>
    </section>
  </div>;
...
function DemoBanner({onBuy,onExit,changed}){
  return <>
...
function AuthScreen({ initialMode="login", initialError="", initialEmail="", onPasswordUpdated }={}){
...
function HomeScreen({ user, hasResults, form, resultToken, onViewResults, onStartNew, onLogout, onGoModule, isDemo=false, onRequestPurchase }){
  ...
  const modules = [
    {emoji:"🎵", label:"Banda sonora", desc:hasResults?`${pareja} · Test completado`:isDemo?"Probá el test; el resultado se desbloquea al comprar":"Creá tu guion musical con IA",
     action:hasResults?onViewResults:onStartNew, status:hasResults?"Ver resultado →":isDemo?"Probar el test →":"Empezar test →",
     done:hasResults, primary:true, locked:false},
    {emoji:"💰", label:"Presupuesto", desc:"Control de gastos por categoría",
     action:premiumAction("budget"), status:premiumStatus, done:false, locked:false},
    {emoji:"🏢", label:"Proveedores", desc:"Cotizaciones y contratos",
     action:premiumAction("vendors"), status:premiumStatus, done:false, locked:false},
    {emoji:"📋", label:"Checklist", desc:"Plan completo de la boda",
     action:premiumAction("checklist-boda"), status:premiumStatus, done:false, locked:false},
    {emoji:"👥", label:"Invitados", desc:"Lista, etiquetas y seating recomendado",
     action:premiumAction("guests"), status:premiumStatus, done:false, locked:false},
    {emoji:"🏛️", label:"Diseño del salón", desc:"Plano, decoración y circulación del evento",
     action:premiumAction("salon-design"), status:premiumStatus, done:false, locked:false},
    {emoji:"⏰", label:"Cronograma", desc:"Timeline del día",
     action:premiumAction("timeline"), status:premiumStatus, done:false, locked:false},
    {emoji:"📖", label:"Guía para novios", desc:"Protocolo, ceremonia y consejos",
     action:premiumAction("guia-novios"), status:premiumStatus, done:false, locked:false},
  ];
  ...
  return <div style={{minHeight:"100svh",display:"flex",alignItems:"flex-start",justifyContent:"center",padding:"clamp(20px,4vw,48px)"}}>
    <div className="fu home-content-card" style={{width:"100%",maxWidth:"min(720px,calc(100vw - 32px))"}}>
      ...
      <div style={{textAlign:"center",marginBottom:28}}>
        ...
        <p className="brand-copy" style={{fontSize:"clamp(.9rem,2vw,1.05rem)",margin:"0 auto",maxWidth:500,lineHeight:1.6}}>
          {isDemo?"Viví la experiencia completa: diseñá el salón, probá el presupuesto y organizá tu boda. Todo lo que hagas en esta prueba es temporal hasta acceder a la versión completa.":"Elegí el módulo con el que querés trabajar hoy."}
        </p>
        {isDemo&&<button className="pbtn" onClick={onRequestPurchase} style={{marginTop:16,padding:"12px 24px",minHeight:48}}>Guardar y publicar mi proyecto →</button>}
        {form?.fechaBoda&&(()=>{
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(min(160px,45vw),1fr))",gap:12,marginBottom:24}}>
        {modules.map(({emoji,label,desc,action,status,done,primary,locked})=>
          <button key={label} onClick={action} style={{
            background:done?"rgba(74,94,58,.08)":primary&&!done?"#4A5E3A":locked?"rgba(251,247,239,.72)":"#FBF7EF",
            border:`0.5px solid ${done?"rgba(74,94,58,.28)":primary&&!done?"#4A5E3A":locked?"rgba(26,26,20,.12)":"rgba(201,169,110,.25)"}`,
            borderRadius:16,padding:"16px 14px",textAlign:"left",cursor:"pointer",
            transition:"all .2s",outline:"none",opacity:locked ? .78 : 1,
...
function TimelineModule({user, form, results, onBack}){
  ...
  useEffect(()=>{
    if(!user) return;
    dataClient(user).from("wedding_data").select("timeline,timeline_aprobacion,vendors").eq("user_id",user.id).maybeSingle()
      .then(({data:row})=>{
  ...
  const save = async(list) => {
    setSaving(true);
    try{
      await dataClient(user).from("wedding_data").upsert({user_id:user.id,timeline:list||events,timeline_aprobacion:aprobacion,updated_at:new Date().toISOString()},{onConflict:"user_id"});
      setSaved(true); setTimeout(()=>setSaved(false),1500);
    }catch(e){}
    setSaving(false);
  };
  ...
  const aprobar = async (quien) => {
    ...
    setAprobacion(next);
    try{
      await dataClient(user).from("wedding_data").upsert({user_id:user.id,timeline_aprobacion:next,updated_at:new Date().toISOString()},{onConflict:"user_id"});
    }catch(e){}
  };
...
export default function App(){
  ...
  if(!user){
    if(view==="auth") return decorate(<AuthScreen initialMode={authInitialMode} initialError={authNotice} initialEmail={initialPurchaseEmail}/>);
    return decorate(<Landing onTry={startDemo} onLogin={()=>openAuth("login")} onBuy={()=>setPurchaseOpen(true)}/>);
  }
